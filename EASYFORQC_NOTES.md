...
| Neon | Postgres database production | Lưu account app, Gmail login user, password hash, Project config, Jira auth đã mã hoá | Database hiện dùng connection string Neon; xem trong Neon dashboard |
| Neon tables | Dữ liệu app | `users`: tài khoản/password hash/Gmail profile; `user_settings`: Project config và Jira auth mã hoá theo từng account | Không sửa tay nếu không cần backup/debug |
